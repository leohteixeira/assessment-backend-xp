...
export class NumberValidator implements Validator {
  ...
  async validate(input: any): Promise<void | string> {
    ...
    if (this.params.minValue) {
      if (input < this.params.minValue) {
        return Invalidation.minValue(this.params.minValue)
      }
    }
    if (this.params.maxValue) {
      if (input > this.params.maxValue) {
        return Invalidation.maxValue(this.params.maxValue)
      }
    }
  }
}
