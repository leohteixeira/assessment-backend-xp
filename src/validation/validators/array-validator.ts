...
export class ArrayValidator implements Validator {
  ...
  async validate(array: any): Promise<void | string> {
    ...
    if (this.params.minLength) {
      if (array.length < this.params.minLength) {
        return Invalidation.minLength(this.params.minLength)
      }
    }
    if (this.params.maxLength) {
      if (array.length > this.params.maxLength) {
        return Invalidation.maxLength(this.params.maxLength)
      }
    }
    if (this.params.itemsValidator) {
      for (let index = 0; index < array.length; index++) {
        const error = await this.params.itemsValidator.validate(array[index])
        if (error) {
          return Invalidation.invalidArrayIndex(index, JSON.stringify(error))
        }
      }
    }
  }
