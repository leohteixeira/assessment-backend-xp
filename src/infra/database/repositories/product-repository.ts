    ...
    RemoveProductRepository
{
  ...
  ): Promise<AddProductRepository.Result> {
    try {
      let product
        ...
        if (params.categoryId) {
          for (const categoryId of params.categoryId) {
            const category = await transactionalEntityManager
              .getRepository(PgCategory)
              .findOne({ where: { id: categoryId } })
            if (!category) {
              throw new DatabaseError.NotFound(
  ...
  async findProducts(
  ...
  ): Promise<FindProductsRepository.Result> {
    const productRepository = await PostgresHelper.getRepository(PgProduct)
    const page =
      params.query.page && params.query.page > 0 ? params.query.page : 1
    ...
    if (
      params.query.search &&
      (params.query.searchValue || params.query.searchValue === 0)
    ) {
      queryBuilder = queryBuilder.where(
        `product.${params.query.search} like :searchValue`,
  ...
  async findProduct(
  ...
  ): Promise<FindProductRepository.Result> {
    const productRepository = await PostgresHelper.getRepository(PgProduct)

    const product = await productRepository.findOne({
      join: {
        alias: 'product',
        leftJoinAndSelect: { categories: 'product.categories' }
      },
    ...
    if (!product) {
      throw new DatabaseError.NotFound(
        `"${params.productId}" could not be found`
      )
    }
  ...
  ): Promise<EditProductRepository.Result> {
    try {
      let result
      await getManager().transaction(async (transactionalEntityManager) => {
        // Find product
        ...
        if (params.categories) {
          product.categories = await transactionalEntityManager
            .getRepository(PgCategory)
            .findByIds(params.categories)
        }
