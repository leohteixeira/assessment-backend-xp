...
export class FindCategoryController implements Http.Controller {
  ...
  async handle (request: FindCategoryController.Request): Promise<Http.Response<FindCategoryController.Response>> {
    try {
      const badParams = await this.validation.validate(request)
      if (badParams) return new HttpError.BadRequest(badParams)

      const result = await this.findCategory.find({ categoryId: request.categoryId })
